...
impl SumsetEpsilonAdditiveAproximation {
    ...
    pub fn new<T: FFTConvoluter>(epsilon_inv: u64) -> SumsetIntervalApproximation<T> {
        SumsetIntervalApproximation::new(epsilon_inv, 1.0 / (epsilon_inv as f64))
    }
}

pub struct SumsetIntervalApproximation<T: FFTConvoluter> {
    start: u64,
    delta: f64,
    _phantom: PhantomData<T>,
}

impl<T: FFTConvoluter> SumsetIntervalApproximation<T> {
...
mod tests {
    use crate::{
        fft::complex::ComplexFFT,
        helpers::{self, test::naive_sumset},
    };

    use super::*;

    fn verify_interval_approximation<T: FFTConvoluter>(set: Vec<u64>, delta: f64) {
    ...
    fn verify_epsilon_additive_approximation<T: FFTConvoluter>(set: Vec<u64>, epsilon_inv: u64) {
        for &i in &set {
    ...
    fn test_interval_approximation() {
        verify_interval_approximation::<ComplexFFT>(vec![5, 6, 7, 8, 9, 10], 0.1);
        verify_interval_approximation::<ComplexFFT>(vec![5, 6, 7, 8, 9, 10], 0.01);
        verify_interval_approximation::<ComplexFFT>(
    ...
    fn test_interval_epsilon_approximation() {
        verify_epsilon_additive_approximation::<ComplexFFT>(vec![6, 7, 8, 9, 10, 11], 6);
        verify_epsilon_additive_approximation::<ComplexFFT>((12..24).collect(), 12);
    }
}
